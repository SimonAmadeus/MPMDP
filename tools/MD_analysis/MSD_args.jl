...
t_fit_start =  round(Int64, l_msd * 0.3) # Starting point for the linear fit
t_fit_end = round(Int64, l_msd * 0.7)    # Ending point for the linear fit

linear_fit = lm(@formula(y ~ x), (x = time[t_fit_start:t_fit_end], y = msd[t_fit_start:t_fit_end]))
slope = coef(linear_fit)[2]
#slope, _ = lm(time[t_fit_start:t_fit_end], msd[t_fit_start:t_fit_end])
