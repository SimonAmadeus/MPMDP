...
function read_lammps_x(filename::AbstractString, ::No_Mol)    
        ...
        for line in eachline(file)
            ...
            if length(a) > 1
                if a[2] == "atoms"
                    ...
                    println("$number_particles particles.")
    ...
    return particle_vec, l_chain, velocity_vec, bond_vec, angle_vec, box
end

# Read input for molecular system.
function read_lammps_x(filename::AbstractString, ::Molecular)    
    
    l_chain = 0 # Chain length - 1 for atoms.
    number_particles = 0
    ...
    open(filename) do file
        for line in eachline(file)
            ...
            if counter >= 2 && counter_check == "Atoms"
                ...
                particle_vec[index] = particle_i
                if counter == (number_particles + 2)
                    l_chain = Int(number_particles / i_mol)
                    counter = 0
                    counter_check = "check"
                end
            end
